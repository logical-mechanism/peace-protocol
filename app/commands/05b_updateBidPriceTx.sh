...
# Usage: ./05b_updateBidPriceTx.sh <new_price_lovelace> [bid_amount_lovelace]
# Example: ./05b_updateBidPriceTx.sh 15000000           (update future price only)
# Example: ./05b_updateBidPriceTx.sh 15000000 20000000  (update future price and bid amount)

if [ -z "${1:-}" ]; then
  echo -e "\033[0;31m Usage: $0 <new_price_lovelace> [bid_amount_lovelace] \033[0m"
  exit 1
fi
...
echo -e "\033[1;36mNew Price: ${NEW_PRICE} lovelace \033[0m"

# build the updated datum with new price
cp ../data/bidding/bidding-datum.json ../data/bidding/next-bidding-datum.json

jq \
--argjson new_price "${NEW_PRICE}" \
'.fields[5].int=$new_price' \
../data/bidding/next-bidding-datum.json | sponge ../data/bidding/next-bidding-datum.json

# build the redeemer with the new price
jq \
--argjson new_price "${NEW_PRICE}" \
'.fields[0].int=$new_price' \
../data/bidding/bidding-update-price-redeemer.json | sponge ../data/bidding/bidding-update-price-redeemer.json

# determine the bid output value
if [ -n "${BID_AMOUNT}" ]; then
  echo -e "\033[1;36mNew Bid Amount: ${BID_AMOUNT} lovelace \033[0m"
  utxo_value=$(${cli} conway transaction calculate-min-required-utxo \
      --protocol-params-file ./tmp/protocol.json \
      --tx-out-inline-datum-file ../data/bidding/next-bidding-datum.json \
      --tx-out="${bidding_script_address} + 5000000 + ${bidding_asset}" | tr -dc '0-9')
  # use the larger of min-utxo or requested bid amount
  if [ "${BID_AMOUNT}" -gt "${utxo_value}" ]; then
    utxo_value=${BID_AMOUNT}
  fi
else
  # keep the same lovelace amount by querying current utxo value
  utxo_value=$(${cli} conway transaction calculate-min-required-utxo \
      --protocol-params-file ./tmp/protocol.json \
      --tx-out-inline-datum-file ../data/bidding/next-bidding-datum.json \
      --tx-out="${bidding_script_address} + 5000000 + ${bidding_asset}" | tr -dc '0-9')
...
FEE=$(${cli} conway transaction build \
    ...
    --spending-reference-tx-in-redeemer-file ../data/bidding/bidding-update-price-redeemer.json \
...
# update the local datum file
cp ../data/bidding/next-bidding-datum.json ../data/bidding/bidding-datum.json
