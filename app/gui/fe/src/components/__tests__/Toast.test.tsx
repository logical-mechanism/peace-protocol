...
describe('Toast transaction celebration', () => {
  ...
  it('applies tx-celebration-icon class to transaction success icon', () => {
    const toasts: ToastMessage[] = [
      { id: '1', type: 'success', title: 'Sale!', duration: 0, variant: 'transaction' },
    ]
    render(<ToastContainer toasts={toasts} onClose={vi.fn()} />)
    const alert = screen.getByRole('alert')
    expect(alert.querySelector('.tx-celebration-icon')).toBeInTheDocument()
  })

  it('does not apply tx-celebration-icon class to regular success', () => {
    const toasts: ToastMessage[] = [
      { id: '1', type: 'success', title: 'OK', duration: 0 },
    ]
    render(<ToastContainer toasts={toasts} onClose={vi.fn()} />)
    const alert = screen.getByRole('alert')
    expect(alert.querySelector('.tx-celebration-icon')).not.toBeInTheDocument()
  })
})
