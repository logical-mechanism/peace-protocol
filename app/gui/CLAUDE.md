...
│   │   ├── contexts/               # WalletContext, NodeContext, WasmContext, ModalContext, AcceptBidQueueContext
...
- `AcceptBidQueueContext` — Queue-based accept-bid orchestration; manages queued/processing/completed/failed states, sequential processing with SNARK proving + re-encryption chaining
...
- SIGTERM → configurable wait → SIGKILL: cardano-node 90s (flush in-memory ledger), mithril-client 30s, others 10s
...
- Liveness monitor: background HTTP health checks at 10s intervals for Ogmios, Kupo, Express; stall detection after 120s of same tip slot; auto-restart on 180s health check failure
...
- Component tests (AudioPlayer, PlaceBidModal) and context tests (WalletContext, NodeContext, WasmContext, ModalContext) use Tauri mock at `fe/src/test/__mocks__/tauri.ts`
...
- **Token names must mirror on-chain `bytearray.push(idx, tx_id)` byte-for-byte** — `computeTokenName` in `fe/src/services/transactions/txUtils.ts` prepends `bytes([outputIndex])` to `txHash` then truncates to 32 bytes. The earlier CBOR-encoded form diverged from on-chain whenever `outputIndex >= 24` (CBOR major-type-0 emits 2 bytes there) and the mint validator's `assets.has_nft_strict` check would reject the tx. The function now throws on `outputIndex` outside `[0, 255]` (matches `bytearray.push` rejection on-chain). See audit finding I-08
- **Continuing protocol UTxOs require `stake_credential = None` and `reference_script = None`** — the v0.6.0 H-01 fix pins the perimeter at every continuing-output site in encryption.ak / bidding.ak. MeshTxBuilder's `.txOut()` already emits bare script-only addresses with no reference script, so the GUI is compliant by default; never wrap a protocol output in an address with a stake credential or attach a reference script (would now fail validation)
- **Contract hashes are env-var loaded, not hardcoded** — `be/src/config/index.ts` reads `ENCRYPTION_POLICY_ID_*`, `BIDDING_POLICY_ID_*`, `GROTH_POLICY_ID_*`, `GENESIS_POLICY_ID_*`, `GENESIS_TOKEN_NAME_*`, and the reference-script UTxO refs from environment per network. After every `app/contracts/compile.sh` run that produces new hashes (e.g. v0.6.0 broke all five hashes), update the env vars at deployment — no GUI source changes needed. The genesis token name must be derived externally using the bytewise form, not CBOR
- **Genesis policy is parameterised at compile time, not at tx-build time** — the GUI never re-applies parameters to genesis. Bootstrap is external (`app/contracts/compile.sh` + `app/setup.sh` + `app/commands/01a_createGenesisTx.sh`); the GUI only references the already-compiled policy_id from env config. Post-v0.6.0 the genesis policy takes 3 params `(tx_id, tx_idx, reference_hash)` instead of 2 — but again this is a deployment-time concern, not a GUI concern
...
- **Provider nesting order** — ErrorBoundary → ShutdownOverlay → WalletProvider → NodeProvider → WasmProvider → AcceptBidQueueProvider → BrowserRouter → ModalProvider → App (in main.tsx); `initializeTheme()` called before `createRoot()` to prevent flash; order matters for context dependencies
...
- **Accept-bid queue** — `AcceptBidQueueContext` + `acceptBidQueueService.ts` + `acceptBidQueueStorage.ts` provide sequential accept-bid processing. Queue items flow through states: queued → processing → completed/failed. Processing involves SNARK proving + chained re-encryption tx. `useAutoAcceptDetection` hook monitors incoming bids and can auto-enqueue. `AcceptBidQueuePanel` component shows queue status in Dashboard/Settings. `AutomationSection` in Settings controls auto-accept behavior. Queue state persisted in localStorage; processing resumes on app restart
